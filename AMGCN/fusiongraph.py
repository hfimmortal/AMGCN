...
class FusionGraphModel(nn.Module):
    def __init__(self, graph, device, conf_graph, conf_data, M, d, bn_decay):
        ...
        if self.graph.shape[0] == 1:
            self.fusion_graph = False
            self.A_single = self.graph
        else:
            self.fusion_graph = True
            self.softmax = nn.Softmax(dim=1)

            if self.matrix_w:
                adj_w = nn.Parameter(torch.randn(self.graph.shape[0], self.graph.shape[1], self.graph.shape[2]))
                adj_w_bias = nn.Parameter(torch.randn(self.graph.shape[1], self.graph.shape[2]))
                self.adj_w_bias = nn.Parameter(adj_w_bias.to(device), requires_grad=True)
                self.linear = linear(5, 1)

            else:
                adj_w = nn.Parameter(torch.randn(1, self.graph.shape[0]))

            self.adj_w = nn.Parameter(adj_w.to(device), requires_grad=True)
            self.used_graphs = self.graph
            assert len(self.used_graphs) == self.graph.shape[0]
...
#        SE = torch.from_numpy(np.float32(pd.read_csv('E:\MLSTGCN-master\MLSTGCN-master\data\SE\se_pm25.csv', header=None).values)).to('cpu')
        # load spatial embedding

        if self.fusion_graph:
            if not self.matrix_w:
                self.A_w = self.softmax(self.adj_w)[0]
                adj_list = [self.used_graphs[i] * self.A_w[i] for i in range(self.graph.shape[0])]
